...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const cosF = Math.acos(x1 * x2 + y1 * y2);
  return cosF / (Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2));
}
...
function getSumOfDigits(num) {
  ...
  let sum = 0;
  while (numFix > 0) {
    const numReminder = numFix % 10;
    sum += numReminder;
    numFix = parseInt(numFix / 10, 10);
  }
  return sum;
}
...
function numberToStringInBase(number, base) {
  const numFix = number.toString(base);
  return numFix;
}
...
function roundToSmallestInteger(number) {
  const numFix = Math.floor(number);
  return numFix;
}
...
function getCountOfOddNumbers(number) {
  const numFix = Math.abs(number);
  let countOdd = 0;
  for (let i = 0; i <= numFix; i += 1) {
    if (i % 2 !== 0) {
      countOdd += 1;
    }
  }
